...
<template>
  ...
  >
    ...
    <div class="absolute top-[16px] right-[16px]">
      <div v-if="selected" class="w-[20px] h-[20px] rounded-full bg-btn-primary flex items-center justify-center">
      ...
      <div v-else class="w-[20px] h-[20px] rounded-full border-2 border-border group-hover:border-border-hover" />
    </div>
    ...
    <div :class="['p-[24px] pt-[20px] w-full', align === 'center' ? 'flex flex-col items-center' : '']">
      ...
      <!-- Title -->
      <div class="text-[16px] font-semibold text-content-primary leading-[24px]">
        <slot />
      </div>

      <!-- Description -->
      <div v-if="$slots.description" class="text-[14px] text-content-secondary mt-[4px] leading-[20px]">
