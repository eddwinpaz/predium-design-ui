...
const calendarDays = computed(() => {
  ...
  for (let i = firstDay - 1; i >= 0; i--) {
    ...
    cells.push({ day: daysInPrev - i, month: (m + 12) % 12, year: y, current: false })
  }

  // Current month
  for (let d = 1; d <= daysInMonth; d++) {
    cells.push({ day: d, month: viewMonth.value, year: viewYear.value, current: true })
  }
...
<template>
  <div ref="containerRef" class="relative inline-block w-64">
    ...
    <button
      ...
      @click="toggle"
    >
      <span :class="modelValue ? 'text-content-primary' : 'text-content-tertiary'">
        {{ displayValue || placeholder }}
      </span>
      <svg class="w-4 h-4 text-content-secondary ml-2 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    >
      ...
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-sm font-semibold text-content-primary">{{ MONTHS[viewMonth] }} {{ viewYear }}</span>
        <button
          type="button"
          class="w-8 h-8 flex items-center justify-center rounded-lg text-content-primary hover:bg-bg-secondary transition-colors cursor-pointer"
          @click="nextMonth"
